  ...
  return (
    <div className="gradient shadow-[0_8px_8px_-8px_#4D323910] sticky top-0 overflow-x-clip z-10">
      <div className={`${styles.innerWidth} flex py-6 justify-between`}>
        ...
        {/* menu button */}
        <button
          className="z-20 text-2xl md:hidden"
        ...
        <AnimatePresence>
          {(isOpen || window.width > 768) && (
            <m.nav
              ...
              className="gradient md:bg-none w-2/3 md:w-auto h-screen md:h-auto md:grid absolute md:static right-0 top-0 py-16 px-8 md:p-0"
            >
              <ul className="flex flex-col md:flex-row md:items-center gap-8 md:gap-4">
                <Menu as="li" className="relative">
                  <Menu.Button className="px-1 flex gap-1 items-center">
                    ...
                    <IconContext.Provider value={{ size: "1.25rem", title: "Choose language" }}>
                      <RiTranslate2 />
                      <RiArrowDropDownLine />
